...
import { createTripSession, generateTripPlan } from '@/services/api'
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (response.success && response.data) {
      const sessionResponse = await createTripSession({
        request: requestData,
        plan: response.data
      })

      if (!sessionResponse.success || !sessionResponse.data) {
        message.error(sessionResponse.message || '创建旅行规划会话失败')
        return
      }

      // 保存到sessionStorage
      sessionStorage.setItem('tripSession', JSON.stringify(sessionResponse.data))
      sessionStorage.setItem('tripSessionId', sessionResponse.data.id)
      sessionStorage.setItem('tripPlan', JSON.stringify(sessionResponse.data.current_plan))
