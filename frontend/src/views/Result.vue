  ...
  <div class="result-container">
    ...
    <div v-if="tripPlan" class="content-wrapper">
      ...
      <div class="main-content">
        ...
        <a-card title="📅 每日行程" :bordered="false" class="days-card">
          <a-collapse v-model:activeKey="activeDays" accordion>
            ...
            >
                  ...
                  <a-list-item>
                    <a-card :title="item.name" size="small" class="attraction-card">
                      ...
                      <div class="attraction-image-wrapper">
                        <a-carousel
                          arrows
                          :dots="getAttractionImages(item).length > 1"
                          class="attraction-carousel"
                        >
                          <div
                            v-for="(image, imageIndex) in getAttractionImages(item)"
                          ...
                          >
                            <img
                              ...
                              @click="openImagePreview(getAttractionImages(item), imageIndex, item.name)"
                              @error="handleImageError"
                            />
                          </div>
                        </a-carousel>
                        <div class="attraction-badge">
                          <span class="badge-number">{{ getAttractionGlobalNumber(day.day_index, index) }}</span>
                        </div>
                      ...
                      <div v-else>
                        ...
                        <div v-if="getPhotoSpotDetails(item).length" class="photo-spot-section">
                          <div class="photo-spot-title">推荐打卡点</div>
                          <div class="photo-spot-grid">
                            <div
                              v-for="(spot, spotIndex) in getPhotoSpotDetails(item)"
                              :key="`${item.name}-${spot.name}`"
                              class="photo-spot-card"
                            >
                              <img
                                :src="getPhotoSpotImage(item, spotIndex)"
                                :alt="spot.name"
                                class="photo-spot-image"
                                @click="openImagePreview(getAttractionImages(item), spotIndex, spot.name)"
                                @error="handleImageError"
                              />
                              <div class="photo-spot-info">
                                <strong>{{ spot.name }}</strong>
                                <span v-if="spot.description">{{ spot.description }}</span>
                              </div>
                            </div>
                          </div>
                        </div>
              ...
              <!-- 酒店推荐 -->
              <a-divider v-if="day.hotel && shouldShowDailyHotel" orientation="left">🏨 住宿推荐</a-divider>
              <a-card v-if="day.hotel && shouldShowDailyHotel" size="small" class="hotel-card">
                <template #title>
              ...
              <div class="meal-grid">
                ...
                >
                  <div class="meal-image-wrapper">
                    <img
                      :src="getMealImage(meal, day.day_index)"
                      :alt="meal.name"
                      class="meal-image"
                      @click="openImagePreview(getMealImages(meal, day.day_index), 0, meal.name)"
                      @error="handleImageError"
                    />
                  </div>
                  <div class="meal-content">
                    <div class="meal-title-row">
                      <a-tag color="green">{{ getMealLabel(meal.type) }}</a-tag>
                      <strong>{{ meal.name }}</strong>
                    </div>
                    <p v-if="meal.description">{{ meal.description }}</p>
                    <p v-if="meal.address"><strong>地址:</strong> {{ meal.address }}</p>
                    <p v-if="meal.recommended_reason" class="meal-reason">
                      <strong>推荐理由:</strong> {{ meal.recommended_reason }}
                    </p>
                  </div>
                </a-card>
              </div>
        ...
        <a-card
          v-if="tripPlan.hotels && tripPlan.hotels.length"
          title="酒店选择"
          style="margin-top: 20px"
          :bordered="false"
        >
          <a-list :data-source="tripPlan.hotels.slice(0, 4)" :grid="{ gutter: 16, column: 2 }">
            <template #renderItem="{ item, index }">
              <a-list-item>
                <a-card size="small" class="hotel-select-card">
                  <template #title>
                    <div class="hotel-select-title">
                      <span>{{ index + 1 }}.</span>
                      <strong>{{ item.name }}</strong>
                    </div>
                  </template>
                  <div
                    class="hotel-select-media"
                    @click="openImagePreview(getHotelImages(item, index), 0, item.name)"
                  >
                    <img
                      :src="getHotelImage(item, index)"
                      :alt="item.name"
                      class="hotel-select-image"
                      @error="handleImageError"
                    />
                    <span v-if="getHotelImages(item, index).length > 1" class="hotel-image-count">
                      1 / {{ getHotelImages(item, index).length }}
                    </span>
                  </div>
                  <a-descriptions :column="1" size="small">
                    ...
                    <a-descriptions-item label="价格范围">{{ item.price_range }}</a-descriptions-item>
                    <a-descriptions-item label="评分">{{ item.rating || '暂无' }}</a-descriptions-item>
                    <a-descriptions-item label="距离">{{ item.distance }}</a-descriptions-item>
                  </a-descriptions>
                  <p v-if="item.review_summary" class="hotel-review">
                    <strong>点评摘要:</strong> {{ item.review_summary }}
                  </p>
                </a-card>
              </a-list-item>
            </template>
        ...
        <a-card title="行程调整" style="margin-top: 20px" :bordered="false">
          <div v-if="tripSession?.messages?.length" class="chat-history">
            <div
              v-for="(item, index) in tripSession.messages"
              :key="index"
          ...
          <a-textarea
            ...
            :rows="3"
            placeholder="例如：把第二天的景点换近一点，或者整体改成更适合老人出行"
          />
          <div class="chat-actions">
            <a-button type="primary" :loading="chatLoading" @click="sendEditMessage">
              发送修改需求
            </a-button>
            <a-button
              v-if="tripSession?.pending_revision_summary"
              danger
              :loading="revising"
          ...
          <p v-if="tripSession?.pending_revision_summary" class="pending-revision">
            待确认：{{ tripSession.pending_revision_summary }}
          </p>
        </a-card>
      </div>
    </div>
    ...
    >
      <div class="image-preview-wrapper">
        <a-button class="preview-arrow preview-arrow-left" shape="circle" @click="previousPreviewImage">‹</a-button>
        <img
          v-if="imagePreviewImages.length"
          :src="imagePreviewImages[imagePreviewIndex] || ''"
          :alt="imagePreviewTitle"
          class="image-preview"
          @error="handleImageError"
        />
        <a-button class="preview-arrow preview-arrow-right" shape="circle" @click="nextPreviewImage">›</a-button>
        <span class="preview-count">{{ imagePreviewIndex + 1 }} / {{ imagePreviewImages.length }}</span>
      </div>
    </a-modal>
  </div>
</template>
...
import { chatWithTripSession, reviseTripSession } from '@/services/api'
import type { Hotel, Meal, TripPlan, TripSession } from '@/types'

const router = useRouter()
const tripPlan = ref<TripPlan | null>(null)
const tripSession = ref<TripSession | null>(null)
...
const hotelPhotos = ref<Record<string, string[]>>({})
const imagePreviewOpen = ref(false)
const imagePreviewImages = ref<string[]>([])
const imagePreviewIndex = ref(0)
const imagePreviewTitle = ref('')
const shouldShowDailyHotel = false
...
onMounted(async () => {
  ...
  if (data) {
    ...
    const sessionData = sessionStorage.getItem('tripSession')
    if (sessionData) {
      tripSession.value = JSON.parse(sessionData)
      tripPlan.value = tripSession.value?.current_plan || tripPlan.value
    }
    // 加载景点图片
...
const loadAttractionPhotos = async () => {
  ...
  tripPlan.value.days.forEach(day => {
    day.attractions.forEach(attraction => {
      ...
      if (attraction.poi_id) params.set('poi_id', attraction.poi_id)
      if (tripPlan.value?.city) params.set('city', tripPlan.value.city)
      const promise = fetch(`http://localhost:8000/api/poi/photo?${params.toString()}`)
        .then(res => res.json())
        .then(data => {
          if (data.success && Array.isArray(data.data?.photo_urls)) {
            attractionPhotos.value[attraction.name] = data.data.photo_urls
          }
        })
        .catch(err => {
  ...
  await Promise.all(
    tripPlan.value.days.flatMap(day =>
      day.meals.map(async meal => {
        const key = `${day.day_index}-${meal.type}-${meal.name}`
        const params = new URLSearchParams({
          name: meal.name,
          category: 'meal'
        })
        if (meal.poi_id) params.set('poi_id', meal.poi_id)
        if (tripPlan.value?.city) params.set('city', tripPlan.value.city)

        try {
          const response = await fetch(`http://localhost:8000/api/poi/photo?${params.toString()}`)
          const data = await response.json()
          if (data.success && Array.isArray(data.data?.photo_urls)) {
            mealPhotos.value[key] = data.data.photo_urls
          }
        } catch (error) {
          console.error(`获取${meal.name}餐饮图片失败:`, error)
        }
      })
    )
  )
...
const getHotelPhotoKey = (hotel: Hotel, index: number): string => {
  return `${index}-${hotel.name}-${hotel.address || ''}`
}

const loadHotelPhotos = async () => {
  if (!tripPlan.value?.hotels?.length) return

  await Promise.all(
    tripPlan.value.hotels.slice(0, 4).map(async (hotel, index) => {
      const key = getHotelPhotoKey(hotel, index)
      const params = new URLSearchParams({
        name: hotel.name,
        category: 'hotel'
      })
      ...
      try {
        ...
        const data = await response.json()
        if (data.success && Array.isArray(data.data?.photo_urls)) {
          hotelPhotos.value[key] = data.data.photo_urls
        }
      } catch (error) {
        console.error(`获取${hotel.name}酒店图片失败:`, error)
      }
    })
  )
}
...
const getAttractionImages = (attraction: any): string[] => {
  if (attractionPhotos.value[attraction.name]?.length) return attractionPhotos.value[attraction.name]
  if (Array.isArray(attraction.photos) && attraction.photos.length) return attraction.photos
  if (attraction.image_url) return [attraction.image_url]
  return [getAttractionImage(attraction.name, 0)]
}

const getAttractionImage = (name: string, index: number): string => {
  // 如果已加载真实图片,返回真实图片
  if (attractionPhotos.value[name]?.length) {
    return attractionPhotos.value[name][0] || ''
  }
  ...
  const { start, end } = colors[colorIndex] ?? { start: '#667eea', end: '#764ba2' }
...
const getMealImages = (meal: Meal, dayIndex: number): string[] => {
  const key = `${dayIndex}-${meal.type}-${meal.name}`
  if (mealPhotos.value[key]?.length) return mealPhotos.value[key]
  if (Array.isArray(meal.photos) && meal.photos.length) return meal.photos
  if (meal.image_url) return [meal.image_url]
  return [getAttractionImage(meal.name, dayIndex)]
}

const getMealImage = (meal: Meal, dayIndex: number): string => {
  return getMealImages(meal, dayIndex)[0] || getAttractionImage(meal.name, dayIndex)
}

const getHotelImages = (hotel: Hotel, index: number): string[] => {
  const key = getHotelPhotoKey(hotel, index)
  if (hotelPhotos.value[key]?.length) return hotelPhotos.value[key]
  return [getAttractionImage(hotel.name, index)]
}

const getHotelImage = (hotel: Hotel, index: number): string => {
  return getHotelImages(hotel, index)[0] || getAttractionImage(hotel.name, index)
}

const getPhotoSpotDetails = (attraction: any): Array<{ name: string; description?: string }> => {
  if (Array.isArray(attraction.photo_spot_details) && attraction.photo_spot_details.length) {
    return attraction.photo_spot_details
      .filter((spot: any) => spot?.name)
      .slice(0, 4)
      .map((spot: any) => ({
        name: spot.name,
  ...
  if (Array.isArray(attraction.photo_spots) && attraction.photo_spots.length) {
    return attraction.photo_spots
      .filter(Boolean)
      .slice(0, 4)
...
const getPhotoSpotImage = (attraction: any, spotIndex: number): string => {
  const images = getAttractionImages(attraction)
  return images[spotIndex % images.length] || getAttractionImage(attraction.name, spotIndex)
}

const openImagePreview = (images: string[], index: number, title: string) => {
  imagePreviewImages.value = images
  imagePreviewIndex.value = Math.max(0, Math.min(index, images.length - 1))
  imagePreviewTitle.value = title
  imagePreviewOpen.value = true
}

const previousPreviewImage = () => {
  if (!imagePreviewImages.value.length) return
  imagePreviewIndex.value =
    (imagePreviewIndex.value - 1 + imagePreviewImages.value.length) %
    imagePreviewImages.value.length
}
...
const getAttractionGlobalNumber = (dayIndex: number, attractionIndex: number): number => {
  if (!tripPlan.value) return attractionIndex + 1
  return tripPlan.value.days
    .slice(0, dayIndex)
    .reduce((total, day) => total + day.attractions.length, 0) + attractionIndex + 1
}

// 图片加载失败时的处理
const syncTripSession = (session: TripSession) => {
  tripSession.value = session
  tripPlan.value = session.current_plan
  sessionStorage.setItem('tripSession', JSON.stringify(session))
  sessionStorage.setItem('tripPlan', JSON.stringify(session.current_plan))
}

const sendEditMessage = async () => {
  if (!tripSession.value) {
    message.warning('当前行程没有会话信息，请重新生成一次行程')
    return
  }
  ...
  try {
    const response = await chatWithTripSession(tripSession.value.id, chatInput.value.trim())
    if (response.success && response.data) {
      syncTripSession(response.data)
      chatInput.value = ''
      await loadAttractionPhotos()
      await loadMealPhotos()
      await loadHotelPhotos()
      message.success(response.message || '修改需求已提交')
      return
    }
...
const confirmRevision = async () => {
  if (!tripSession.value) return

  revising.value = true
  try {
    const response = await reviseTripSession(tripSession.value.id)
    if (response.success && response.data) {
      syncTripSession(response.data)
      ...
      await nextTick()
      initMap()
      message.success(response.message || '行程已重新规划')
      return
    }
    message.warning(response.message || '重新规划失败')
  } catch (error: unknown) {
...
.attraction-carousel,
.attraction-carousel :deep(.slick-list),
.attraction-carousel :deep(.slick-track),
.attraction-carousel :deep(.slick-slide > div) {
  height: 200px;
}
